#!/usr/bin/env python3
"""
run.py — Start the TRU Policy Assistant server.
...
def main():
    parser = argparse.ArgumentParser(description="Start the TRU Policy Assistant")
    ...
    log.info("Starting TRU Policy Assistant on http://%s:%s", display_host, args.port)
    uvicorn.run(
        "app.server:app",
        host=args.host,
        port=args.port,
